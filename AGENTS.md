...
Run `bun run i18n:extract` to extract messages into `.po` catalogs.
...
- `bun run zero:generate` — generate Zero schema from Drizzle
- `bun run lint` — run ESLint
- `bun run format` — format with Prettier
- `bun run i18n:extract` — extract messages to .po catalogs
- `bun run test` — run tests (requires Docker for testcontainers)
